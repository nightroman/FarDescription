...
static class Description
{
	...
	static readonly WeakReference WeakCache = new(null);
	...
	internal static Dictionary<string, string> Import(string descriptionFile)
	{
		...
		using (var sr = new StreamReader(descriptionFile, encoding, true))
		{
			string line;
	...
	internal static void Export(string descriptionFile, Dictionary<string, string> map)
	{
		lock (WeakCache)
		{
			...
			// write
			int nWritten = 0;
			using (var sw = new StreamWriter(descriptionFile, false, encoding))
			{
